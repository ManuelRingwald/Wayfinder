...
| 2 | **Observability-Grundgerüst** ⏳ *in Arbeit* | Firefly + Wayfinder | **Häppchen 2.1 ✅ erledigt** (Wayfinder: totes `internal/config` entfernt, `WAYFINDER_LOG_LEVEL` konfigurierbar, Client-Eviction im Broadcaster geloggt, NFR-OBS-001); **2.2** Firefly — `tracing` in `firefly-multicast`/`firefly-asterix` einziehen (Decode-Fehler, Socket-Fehler, offen); **2.3** gemeinsam — `/metrics`-Endpoint (Prometheus) für Track-Zahlen, Decode-Fehler, Drops, Client-Counts (offen) | **S3 · Sonnet 4.6** |
