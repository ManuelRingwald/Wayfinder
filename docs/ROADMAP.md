...
| **ASD-013** ✅ | Alarm-/Ereignis-Panel (Feed-stale/degraded/recovered, Connection lost/restored, Track appeared/disappeared) | **S3 · Sonnet 4.6** | **erledigt** (FR-UI-027) | Rein Frontend, aus dem WS-Strom abgeleitet (reine Ableitung `map/events.js` + Ring-Puffer-Store + `EventPanel.vue`, Glocke mit Ungesehen-Badge). Automatisch **mandanten-skopiert** (der Strom ist es, WF2-21). Prime-Logik gegen „appeared"-Flut nach (Re)Connect; „beendet" nur per TSE (kein Scan-Ausfall-Rauschen). Ehrliche Grenze: keine Wire-Alarme (STCA/Militär draußen, vgl. ASD-006). Kein CAT062-Bezug. |
| **ASD-014** | **AoR-Overlay-Ebene** (ADR 0021, Ebene 2): flughafengebundene, hervorgehobene **Verantwortungsbereiche** (CTR/TMA) aus **OpenAIP**. Auswahl-Semantik = explizite, pro-Mandant konfigurierte **`_id`-Liste** (Option 1). Vorbau: OpenAIP-Transform (`pkg/aeronautical`) um `_id`/Floor-Ceiling/`icaoClass` erweitern. | **S3–S4 · Opus 4.8** | offen | **Pro Mandant** (`view_configs`) → mandanten-gescopt. Kleiner Backend-Anteil (View-Config + `/api/airspace`-Tagging) + Frontend-Highlight. Kein CAT062-Schnittstellen-Bezug. |
| **ASD-015** | **DFS-AIXM als autoritative Produktionsquelle** für AoR-/Luftraum-Geometrie (Ergänzung/Ablösung OpenAIP). Backend-Pipeline **AIXM 5.1.1 → GeoJSON** (GDAL/GMLAS, Kurven-Densifizierung, AIRAC-gepinnt). | **S4–S5 · Opus 4.8 / Fable 5** | offen ([#215](https://github.com/ManuelRingwald/Wayfinder/issues/215)) | „Später/Produktion", **nicht** im kritischen 2.0-Pfad. **Backend-Ingest** (nicht rein Sicht). Lizenzklärung DFS nötig (Redistribution/kommerziell). ADR 0021, Datenquellen-Bewertung C. |
