...
| AP | Inhalt | Stufe · Modell | Abh. | Status |
|----|--------|----------------|------|--------|
| **ORCH-0** 🔒 | **ADR 0012** „Mandanten-eigene Tracker-Instanzen & Auto-Orchestrierung" (`docs/decisions/0012-mandanten-tracker-orchestrierung.md`) — ratifiziert Option A, Firefly-Autonomie (Ports & Adapters, keine Tenant-Kenntnis), Coverage-BBox-in-Firefly vs. autoritative-AOI-in-Wayfinder, `InstanceBackend` (Docker→K8s), Reconciler-Lebenszyklus am Feed, Sicherheits-/Control-Plane-Grenze | **S4 · Opus 4.8** | — | 📝 **ENTWURF vorgelegt** — wartet auf Freigabe |
