...
- **Zuletzt aktualisiert:** 2026-06-28 — **Strategischer Prioritäts-Rahmen gesetzt (Prio 1 / Prio 2) + ADR 0013 „Modular CWP & Enterprise ATC Integration" akzeptiert (Planung, kein Produktivcode).** Branch `claude/kind-feynman-kr2co1`. **(1) ADR 0013** (`docs/decisions/0013-modular-cwp-enterprise-atc-integration.md`) ratifiziert die Vision, Wayfinder vom isolierten ASD zur modularen **CWP-Suite (ASD + EFS + IMS)** auszubauen — sechs Entscheidungen: D1 drei komponierbare Module + Shell (standalone/split/multi-monitor, kein Monolith-Store), D2 Client-Koordination über native **`BroadcastChannel`** (kein Backend-UI-Highlighting; Korrelations-Vertrag ICAO→Callsign→Track-Nr.; Session-Guard), D3 EFS = zustandsbehaftete Strips mit **deterministischer, auditierter FDP-State-Machine** + Handover, D4 **Workstation/Rollen-Modell** (Login-Kontext = Mandant + operative Rolle + Arbeitsplatz; zwei orthogonale Rollen-Achsen), D5 **IMS auf SWIM** (AMQP 1.0, AIXM/FIXM/IWXXM, FAA-SCDS-validierbar), D6 drei getrennte Server-Planes + `tenant_id` bleibt autoritativ + untrusted-SWIM-Ingress. **Drei offene Punkte ratifiziert:** NATS+AMQP-Doppelbetrieb akzeptiert (kein Architektur-Kompromiss), Multi-Monitor-Grenze = ein Browser/ein PC, Reihenfolge = Prio 2. **CAT062-Draht-Vertrag mit Firefly bleibt unverändert.** **(2) ROADMAP.md** um einen **verbindlichen Prioritäts-Rahmen** ergänzt (strikt geordnet): **🥇 PRIO 1 — SaaS-/„Netflix"-Fundament:** Zero-Touch & Admin-UI (**Epic ONB / ADR 0011: ✅ vollständig** — Mandanten/Feeds/Feature-Toggles/Nutzer-CRUD live aus der UI, OpenAIP pro Mandant) **+ Auto-Orchestrierung** (**Epic ORCH / ADR 0012:** ORCH-0 ✅, **ORCH-1…6 offen** — „Feed zuweisen ⇒ Firefly-Instanz startet automatisch"). **🥈 PRIO 2 — Modular CWP** (Epic CWP/EFS/IMS, ADR 0013) — erst nach Prio 1. Kein Vorziehen von Prio-2-Paketen. **Nächster konkreter Schritt (Prio 1):** **ORCH-1 (Feed-Quell-Datenmodell)** — `source_config`/`coverage_bbox`, Migration, Admin-API, UI-Quell-Builder; nach Ankündigung & „Go" bauen (S3–S4 · Sonnet/Opus).
- **Vorherige Aktualisierung:** 2026-06-26 — **Planung: Epic ORCH „Mandanten-eigene Tracker-Instanzen & Auto-Orchestrierung" in die Roadmap aufgenommen (Planung, kein Produktivcode).** Branch `claude/kind-feynman-kr2co1`. **Betreiber-Entscheidung:** Sensor-Trennung pro Mandant wird über **eine dedizierte Firefly-Instanz pro Mandant** gelöst (vormals „Option A"), **nicht** per Wayfinder-Track-Heuristik. Leitprinzip: **Firefly bleibt autonomer, generischer Tracker (ARTAS-artig), keine Wayfinder-Spezialfälle** — Wayfinder wird zusätzlich **Orchestrator** (Feed-Zuweisung startet automatisch die passende Firefly-Instanz). **Einwand zur AOI-Frage festgehalten:** Coverage-/Quell-BBox (z. B. OpenSky-Abfrage) gehört als **generische** Konfig nach Firefly (grobe äußere Grenze); die **autoritative** Anzeige-/Isolations-AOI (Kreis+Radius+FL) bleibt in Wayfinder (präziser innerer Filter) — komplementär, keine doppelte Logik, keine Tenant-Kenntnis in Firefly. **ADS-B-Quelle:** Firefly-Input-Adapter `adsb_opensky` (REST `/states/all` mit BBox), erweiterbar auf `flarm_aprs`/`radar_asterix` (Ports & Adapters). **Roadmap §Stufe 6:** ADR 0012 (ORCH-0) + ORCH-1…6 (Feed-Quell-Datenmodell, `InstanceBackend` Docker→K8s, Reconciler am Feed-Lebenszyklus, Orchestrierungs-UX, Firefly-Cross-Project-Ingestion, Skalierung/HA). 🔒 Sicherheits-Leitplanke: Container-Start = getrennte Least-Privilege-Control-Plane, nicht im browser-zugewandten Prozess. Frühere „Option B/C" (Per-Track-Sensorfilter, ICD-`source_type`) **verworfen**. **Außerdem in dieser Sitzung:** veralteten `super_admin`-Kommentar in `AdminProvisioning.vue` → `admin`/`requireAdmin` korrigiert (Commit `e3021b4`); Feed-/Sensor-Mix-Fragen im Chat geklärt (Sensor-Mix = reines Metadatum, kein Track-Filter). **ADR 0012 (ORCH-0) als ENTWURF ausgearbeitet** (`docs/decisions/0012-mandanten-tracker-orchestrierung.md`): 6 Entscheidungen (Sensor-Trennung an der Quelle, Firefly-Autonomie/Ports & Adapters, Coverage-vs-AOI-Trennung, `InstanceBackend` Docker→K8s, Reconciler am Feed-Lebenszyklus, getrennte Least-Privilege-Control-Plane), Verworfene Alternativen (Option B/C, AOI-in-Firefly, doppelte AOI), ORCH-0…6-Tabelle, ehrliche Grenzen (OpenSky kein ATC-grade, N Instanzen = Ressourcenhunger, Firefly-Live-Quellen = größte Abhängigkeit). **Nächster Schritt:** Freigabe von ADR 0012 abwarten, **dann** ORCH-1 (Feed-Quell-Datenmodell) ankündigen & bauen. Bis dahin alternativ Betriebs-Härtung.
- **Vorherige Aktualisierung:** 2026-06-26 — **Zero-Touch-Onboarding ONB-6 Frontend (S2–S3 · Opus 4.8) — Epic vollständig.** Branch `claude/kind-feynman-kr2co1`. Zweiter ONB-6-Commit: OpenAIP-Konfiguration in der UI. **Store** (`admin.js`): `loadTenantOpenAIP(id)` (GET → nur `{configured}`, **nie** der Schlüssel), `setTenantOpenAIPKey(id, apiKey)` (PUT; String = setzen, `null` = löschen; Banner „gespeichert"/„entfernt"). **`AdminTenantDetail.vue`** (neuer Abschnitt „OpenAIP-Konfiguration"): Status-Chip (gesetzt / globaler Schlüssel), maskiertes Eingabefeld für einen **neuen** Schlüssel (Sicht-Umschalter), „Speichern"/„Entfernen"; der Schlüssel wird nie vorbefüllt/angezeigt, Status nach jeder Mutation neu geladen. **Gates:** `vitest` ✅ (161, +4 ONB-6-Tests), `npm build` ✅ (`dist/` aktualisiert). Doku: Milestone `ONB-6_OpenAIP_Per_Tenant.md` (Frontend-Abschnitt), FR-ADMIN-009 (Frontend-Verweis), STATUS. **Damit ist das Zero-Touch-Onboarding-Epic (ADR 0011, ONB-0…ONB-6) vollständig** — eine frische Instanz ist ohne Terminal-Schritt vollständig über die Oberfläche betreibbar (Admin-Seed + Pflichtwechsel, Selbstverwaltung, Admins/Mandanten/Feeds-CRUD live, OpenAIP pro Mandant). **Nächster Schritt:** Betriebs-Härtung (Observability-Ausbau, Lastfestigkeit, Mehr-Replica-Konfig-Watch) oder neues Paket nach Ankündigung & Freigabe.
- **Vorherige Aktualisierung:** 2026-06-26 — **Zero-Touch-Onboarding ONB-6 Backend (S4 · Opus 4.8) — letztes Paket des Epics.** Branch `claude/kind-feynman-kr2co1`. **OpenAIP pro Mandant:** jeder Mandant nutzt einen **eigenen optionalen API-Schlüssel** und ruft Luftraumdaten gegen **seine eigene AOI** (aus der View-Konfig) ab; Mandanten ohne Schlüssel fallen auf den globalen `WAYFINDER_OPENAIP_API_KEY` zurück. **Schema:** Migration `00009_tenant_openaip_key.sql` (`tenants.openaip_api_key TEXT` nullable). **Store:** `TenantRepo.GetOpenAIPKey`/`SetOpenAIPKey` (Schlüssel isoliert, **nie** Teil der `Tenant`-Zeile/DTO). **`pkg/aeronautical.Registry`** (neu, Muster wie Feed-Manager): Supervisor mit Map `tenantID→Service`, `Start` (idempotent auf unveränderte Schlüssel+BBox)/`Stop`/`StopAll`/`Serve` (Fallback auf globalen Cache)/`Register` (tenant-aware Endpunkte)/`FetchSuccessCount` (summiert global + alle Mandanten); injizierte `ClientFactory` → transport-entkoppelt, `-race`-getestet. `Service.Serve(kind)` faktorisiert. **Endpunkte** `/api/airspace|navaids|waypoints` im Multi-Tenant **hinter der Tenant-Middleware**, liefern den Cache des Request-Mandanten (kein Identity → leer); Single-Tenant unverändert global. **API (hinter `requireAdmin`):** `GET /api/admin/tenants/{id}/openaip` → `{configured}` (**nie der Schlüssel**); `PUT …/openaip` `{api_key|null}` → 204 (leer/null = löschen), Live-Apply. **Live-AOI:** `putView`/`putTenantView` lösen zusätzlich `triggerAeroApply` aus; **Mandant-Löschen** → `triggerAeroStop`. **Boot:** je Mandant `Apply` (Caches warm). **Entkopplung:** `adminapi.TenantAeroLifecycle` (Primitiv-Schnittstelle); Adapter `tenantAeroLifecycle` in `cmd/wayfinder/aero.go` (effektiver Schlüssel + AOI-Auflösung). **Metriken** `wayfinder_openaip_fetch_*` summiert über Churn. **Gates:** `scripts/pg-test.sh -p 1 ./...` (real-PG, inkl. `TestIntegrationTenantOpenAIPKey`) ✅, `pkg/aeronautical -race` ✅, `go vet`/`gofmt` ✅. Doku: **FR-ADMIN-009**, Milestone `ONB-6_OpenAIP_Per_Tenant.md`, TECHNICAL (Endpunkte + Metrik-/Env-Semantik), INSTALLATION 8.3 (Per-Mandant-Kasten). **Nächster Schritt: ONB-6 Frontend** (Store-Action `setTenantOpenAIPKey` + OpenAIP-Abschnitt in `AdminTenantDetail.vue` + vitest — eigener Commit). Danach ist das **Zero-Touch-Epic vollständig**.
- **Vorherige Aktualisierung:** 2026-06-26 — **Zero-Touch-Onboarding ONB-5 Frontend (S2–S3 · Opus 4.8).** Branch `claude/kind-feynman-kr2co1`. Zweiter ONB-5-Commit: Feed-Verwaltung in der UI. **Store** (`admin.js`): `createFeed(payload)` (POST; 409-Duplikatname → „… Ein Feed mit diesem Namen existiert bereits."), `deleteFeed(id)` (DELETE). **`AdminFeeds.vue`** (neu, Vorlage `AdminPlatformAdmins.vue`): Katalog-Tabelle (Name, `multicast_group:port`, Sensor-Mix-Chips, **Gesundheits-Chip** grün/gelb/rot aus `feedsHealth`); „Feed anlegen"-Dialog (Name, Multicast-Gruppe, Port, optionaler Sensor-Mix) + Lösch-Bestätigung (Hinweis: Abos kaskadieren); lädt nach Mutation neu. **Navigation** (`AdminView.vue`): dritter Header-Toggle-Eintrag **„Feeds"** (Mandanten ↔ Feeds ↔ Plattform-Administratoren). **Gates:** `vitest` ✅ (157, +4 ONB-5-Tests), `npm build` ✅ (`dist/` aktualisiert). Doku: Milestone `ONB-5_Feed_Lifecycle.md` (Frontend-Abschnitt), FR-ADMIN-008 (Frontend-Verweis), STATUS. **Nächster Schritt: ONB-6** (OpenAIP pro Mandant: per-Mandant-API-Key + AOI-Cache, S4 · Opus) nach Ankündigung & Freigabe — **letztes Paket des Zero-Touch-Epics**.
- **Vorherige Aktualisierung:** 2026-06-26 — **Zero-Touch-Onboarding ONB-5 Backend (S4 · Opus 4.8).** Branch `claude/kind-feynman-kr2co1`. Feeds **anlegen/löschen** über die API — **und ohne Neustart**: der Server tritt der Multicast-Gruppe eines neuen Feeds **sofort** bei und verlässt die Gruppe eines gelöschten Feeds **sofort**. **Feed-Manager** (`pkg/feedmanager`, neu): Supervisor mit mutex-geschützter Map `feedID→cancel`, `Start`/`Stop`(wartet auf sauberes Leave)/`StopAll`/`Running`; `Factory` injiziert → **UDP-frei unit-testbar** (`-race`). **Receiver-Härtung** (`pkg/receiver`): Watchdog setzt bei ctx-Cancel eine vergangene Read-Deadline → blockiertes `ReadFromUDP` kehrt **sofort** zurück, IGMP-Leave prompt auch bei totem Feed. **Store:** `FeedRepo.Delete` (atomar, `ON DELETE CASCADE` auf Abos), `GetByName`; Migration `00008_feeds_name_unique.sql` (UNIQUE `name`). **API (hinter `requireAdmin`):** `POST /api/admin/feeds` `{name, multicast_group, port, region?, sensor_mix?}` → 201 (Multicast-IPv4 + Port validiert, Vokabular-`sensor_mix`, doppelter Name → 409; **atomar über Katalog + Live-Join**, Rollback bei Join-Fehler); `DELETE /api/admin/feeds/{id}` → 204 (Receiver verlässt Gruppe, dann Zeile; **Guard C**: kein Blockieren bei Abos — Grants kaskadieren). `feedDTO` um `multicast_group`/`port` erweitert. **main.go:** statischer `buildReceivers`-Block durch Feed-Manager ersetzt; Decode-Error-Zähler **prozessweit-monoton** über Churn (Hook `OnDecodeError`); Staleness-Monitor über lebende Menge (`feedManager.Running`); `health.Registry.Forget` beim Löschen; **ein einzelner Receiver-Fehler beendet nicht mehr den Server**. `adminapi.FeedLifecycle` ist Primitiv-Param-Schnittstelle (kein Transport-Import); Adapter in `feeds.go`. **Gates:** `go test -p 1 ./...` gegen real-PG ✅ (inkl. `TestIntegrationFeedDeleteCascades`/`…NameUnique`), `pkg/feedmanager -race` ✅, `go vet`/`gofmt` ✅. Doku: **FR-ADMIN-008**, Milestone `ONB-5_Feed_Lifecycle.md`, TECHNICAL (Routen), INSTALLATION 5.5 (UI-Hinweis). **Nächster Schritt: ONB-5 Frontend** (Feed-Verwaltungs-UI `AdminFeeds.vue` + Store-Actions `createFeed`/`deleteFeed` + Navigation + vitest — eigener Commit), dann ONB-6 (OpenAIP/Mandant).
