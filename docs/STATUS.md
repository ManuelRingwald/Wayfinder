...
- **Zuletzt aktualisiert:** 2026-06-23 — **Anforderung 1 „Admin-Einstieg in der Navigation Rail" abgeschlossen (S2 · Opus 4.8).** Die Navigation Rail trägt unten einen ans Schienen-Ende gepinnten **Admin**-Eintrag (`mdi-shield-account`), **ausschließlich** sichtbar für `tenant_admin`/`super_admin`; Klick navigiert per Vue Router auf `/admin`. Sichtbarkeit **strikt an die server-autoritative Identität gekoppelt** (`/api/admin/whoami`, einmalig in `onMounted`, **fail-closed**: Operator/401/403/404 → unsichtbar). Store `admin.js` exponiert neu `isAdmin` (`role === 'tenant_admin' || 'super_admin'`); `NavigationRail.vue` bekommt `goAdmin()` + Rail-Button (Desktop) + gegatetes `v-list-item` (Mobile). **Reine Kosmetik-Gating** — der Server erzwingt `/api/admin/*` via `RequireRole`. **Kein Backend-/ICD-Change.** **Gates:** `vitest run` ✅ (82, inkl. neuem `isAdmin`-Matrix-Test), `npm run build` ✅. Branch `claude/admin-rail-button` (frisch von `main`), Register **FR-UI-012**. **Nächster Schritt:** Branch-Wechsel zu **Anforderung 2** (Cross-Tenant Read-Only / Impersonation) — dort **zwingend zuerst ADR 0022-Entwurf** zur Freigabe, bevor Produktivcode geändert wird.
