...
- **Vorherige Aktualisierung:** 2026-06-22 — **ASD-012 „Range-Rings + Scale-Bar +
  ...
  Branch `claude/asd-012-range-rings` (off `main`). **(1) Range-Rings:**
  `rangerings.js` erzeugt konzentrische Kreise **konstanter Boden-Distanz** um den
  Karten-Mittelpunkt (`/api/map-config`) — **geodätisch** (Destination-Point),
  **anti-squish im Test bewiesen**. **Live operator-konfigurierbar:** Abstand
  5/10/15 NM + Anzahl als reaktiver Pinia-State (`rangeRingConfig`, Default 10 NM/5),
  Sidebar-Toggle + Select/Slider; MapCanvas-Watcher → `engine.updateRangeRings`.
  **(2) Scale-Bar:** MapLibre `ScaleControl{unit:'nautical'}` (unten-links).
  **(3) Nord-Kompass:** `NavigationControl{showCompass}` (oben-links); **alter
  Reset-Nord-Button entfernt**. **Tests:** `rangerings.test.js` (9, inkl.
  Anti-Squish), `vitest` 89 grün, `build` grün. Commits `9c0d0ca`/`a8d39ec`/`bc4e533`.
