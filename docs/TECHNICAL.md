...
| `/api/basemap/search` | GET | **FR-UI-037 (#277, ADR 0028):** Sektor-Suche über die Vektor-Tile-Namen der Basiskarte (`?q=…`, min. 2 Zeichen). Server-seitiger, **lazy** je AOI gebauter Index (`pkg/basemapsearch`): z14-Tiles der effektiven View-AOI (ohne AOI: 30-NM-Box ums View-Zentrum), MVT-dekodiert, benannte Features normalisiert (ä→ae/ß→ss/„straße|str." → `str`) und gleichnamig im 3-km-Umkreis geclustert. Antworten: `202 {status:"building"}` während des Baus (Single-Flight; UI pollt), `200 {status:"ready", results:[{name,category,lat,lon,near?,dist_nm?,bearing_deg?}]}` mit max. 20 Treffern (Präfix vor Infix; `near` = nächstgelegener Ort ≤ 8 km, best-effort je Tile-Schema, `dist_nm`/`bearing_deg` = Radial vom Sektorzentrum — Ortsmerkmale zum Unterscheiden gleichnamiger Treffer, #277-Nachtrag), `200 {status:"error"}` wenn der Erst-Bau fehlschlug (Server retried im Hintergrund, Status bleibt stabil bis zum ersten Erfolg; UI zeigt „nicht verfügbar" und pollt gedrosselt weiter), `503` ohne auflösbares Suchgebiet. Der Stil darf seine Vektor-Quelle inline (`tiles`) **oder** als TileJSON-Verweis (`url`) deklarieren — die echten basemap.de/world-Styles nutzen die TileJSON-Form (Betreiber-Befund 2026-07-19). Hinter der Tenant-Middleware; **Feature-Gate `basemap` fail-closed → `403`** (der Index-Bau kostet reale Ressourcen — tausende Tile-Fetches — und ist kein rein kosmetisches Gate). Limits: 4096 Tiles je Index (übergroße AOI Zentrum-erhaltend geclampt), 8 Indexe (LRU), 250 k Einträge, 4 MiB je Tile, Build-Timeout 5 min, TTL 24 h mit Stale-Serve + Hintergrund-Rebuild. Impersonation wie `/api/airspace` (Ziel-Mandant zählt) |
...
| `/api/weather/qnh` | GET | **WX-B (ADR 0016; per-Tenant CBD-3):** aktuelles QNH (hPa, ganzzahlig) des **eigenen** Mandanten-Flugplatzes (`view_configs.qnh_icao`, aus dem Tenant-Kontext aufgelöst) aus NOAA/AWC-METAR (`{stations:[{icao,qnh_hpa,obs_time,stale}],primary?}`). Mandant ohne eigenen Flugplatz → deprecated globaler Fallback. QNH nur aus echtem METAR (`altim`), **nie** DWD-PMSL. Hinter der Tenant-Middleware; best-effort (leere Liste wenn aus/ungesetzt), nie ein Fehler. Kopfzeilen-Anzeige per Entitlement `qnh` in der UI gegated. **Impersonation (ADR 0008 Nachtrag):** bei aktivem Grant zählt der Flugplatz des **Ziel-Mandanten** |
| `/api/weather/warnings.geojson` | GET | **WX-C (ADR 0016):** amtliche DWD-Warnpolygone aus dem GeoServer-WFS (`dwd:Warnungen_Gemeinden_vereinigt`), normalisiert (`wf_level` 1–4 + `headline`/`event`/`expires`), WGS84-GeoJSON. Hinter der Tenant-Middleware; best-effort (leere FeatureCollection wenn aus/Ausfall), nie ein Fehler. Overlay per Entitlement `weather_warnings` in der UI gegated. **#190:** das Frontend clippt die Polygone zusätzlich auf die Mandanten-AOI (aus `whoami`), damit ein landesweit verschmolzenes Warngebiet auf den Sektor zugeschnitten wird |
| `/api/airports.geojson` | GET | **#192:** Flughafen-Referenzpunkt-Marker (GeoJSON Points `{icao,name}`) aus dem eingebetteten Offline-OurAirports-Verzeichnis (`pkg/airport`), begrenzt auf die effektive View-AOI (sonst Radius-Box), max. 400. Hinter der Tenant-Middleware; **Feature-Gate:** ohne `airport`-Entitlement → **leere** Collection. Impersonation wie `/api/airspace` |
| `/api/runways.geojson` | GET | **#192:** Runway-Mittellinien (GeoJSON LineStrings `{icao,ident}`, LE→HE-Schwelle) aus dem eingebetteten Offline-OurAirports-Verzeichnis (`pkg/airport/runways.tsv`), begrenzt auf die effektive View-AOI (sonst Radius-Box), max. 400. Hinter der Tenant-Middleware; **Feature-Gate:** ohne `runways`-Entitlement → **leere** Collection. Impersonation wie `/api/airspace` |
| `/api/whoami` | GET | **WF2-12.4:** rollen-agnostische Identitäts-Probe (`{subject, tenant_id, user_id, role, must_change_password, features, sensor_classes, fl_min?, fl_max?, icao?}`); hinter der Tenant-Middleware, **nicht** `requireAdmin` — die ASD-Karte entscheidet damit Login-Schirm vs. Live-Bild und gated Layer/Legende (Issues #106/#107); `401` ohne Sitzung. `sensor_classes` ist die Vereinigung der Sensor-Klassen über die abonnierten Feeds des Mandanten. `fl_min`/`fl_max` spiegeln das FL-Band der effektiven Ansicht (Standard-Ansicht oder Nutzer-Override) für den grauen Bereichs-Hinweis im FL-Filter der Sidebar (#116; `omitempty`, fehlen wenn kein Band konfiguriert). `icao` (Reskin 3a, FR-UI-020) ist das optionale **ICAO-Kürzel** der effektiven Ansicht (Sektor/FIR, z. B. `EDGG·KTG`), das die ASD-Kopfzeile zeigt — reine Anzeige-Config (kein CAT062-Feld), am View-Config gepflegt (Migration 00015, Admin-View-Editor), `omitempty`. `center_lat`/`center_lon`/`zoom` (FR-UI-013) sind der **Start-Kartenausschnitt** der effektiven Ansicht: die ASD-Karte öffnet auf **diesem** Zentrum, damit der Lotse auf **seinem** Sektor landet statt auf dem globalen `WAYFINDER_MAP_CENTER_*`-Default; `omitempty` — fehlt eine View-Config, greift die globale Env aus `/api/map-config`. `aoi` (#189/#190; `{min_lat,min_lon,max_lat,max_lon}`, `omitempty`) ist die effektive View-AOI, auf die das ASD die DWD-Wetter-Overlays (Regenradar-Raster + Warnpolygone) clippt, damit nur Wetter im eigenen Sektor erscheint. `aor_airspace_ids` (ASD-014, ADR 0021; Array stabiler OpenAIP-Luftraum-`id`s, `omitempty`) ist der **Verantwortungsbereich** (Area of Responsibility) der effektiven Ansicht — die Lufträume (CTR/TMA), die das ASD hervorhebt; am View-Config gepflegt (Migration 00021, editierbar über `PUT /api/admin/view` bzw. `…/tenants/{id}/view`; die id ist die stabile Referenz, weil Luftraum-Namen pro AIRAC driften). **Impersonation (ADR 0008 Nachtrag):** bei aktivem Grant lösen sich `features`/`sensor_classes`/`fl_min`/`fl_max`/`icao` gegen den **Ziel-Mandanten** auf und `impersonated_tenant_id` legt das offen (`omitempty`); die Identitätsfelder (`subject`/`tenant_id`/`user_id`/`role`) bleiben die **echten** des Admins. |
...
| `/api/admin/whoami` | GET | Rollen-Probe + **effektive Feature-Flags** (`features`) als JSON; enthält seit ONB-1 `must_change_password` und `email` (#319); rollen-gegated (WF2-32/50) |
...
| `/api/account/password` | PUT | **#319:** eigenes Passwort ändern (`{current_password, new_password}`, min. 8, aktuelles Passwort falsch → 401); **rollen-agnostisch** (self-scoped, User-ID aus der Session), hinter `tenantMW`+`pwGate` — **nicht** dem Admin-Gate. Gleiche Logik wie `/api/admin/me/password`, aber für **jeden** angemeldeten Nutzer (Lotse im ASD) |
| `/api/account/email` | PUT | **#319:** eigene E-Mail setzen/löschen (`{email}`; leer = löschen; Format-validiert `^[^\s@]+@[^\s@]+\.[^\s@]+$`, ≤ 254). Self-scoped wie oben. Die geänderte E-Mail erscheint in der Admin-Zugangstabelle (`AdminUsers.vue`, gleiches Store-Feld); `GET /api/whoami` trägt `email` zur Anzeige/Vorbelegung |
...
| `/api/admin/airac` | GET | **AERO-3 (ADR 0018):** aktueller AIRAC-Zyklus + nächster Stichtag (`{ident, effective, next_ident, next_effective, days_until_next}`). **Deterministisch** (28-Tage-Raster, Anker 2020-01-02 = „2001"), keine externe Quelle. **admin** |
| `/api/admin/tenants/{id}/openaip/changes` | GET | **AERO-3:** Change-Impact des letzten Abrufs je Ebene (`[{kind, feature_count, prev_feature_count?, added?, removed?, fetched_at}]`); Count-Delta exakt, `added`/`removed` = Inhalts-Hash-Churn (In-Place-Edit = −1/+1); `null` = Erstbefüllung; 404 wenn nicht verdrahtet. **admin** |
| `/api/admin/tenants/{id}/airspaces` | GET | **ASD-014 (ADR 0021):** Luftraum-Liste des Mandanten aus dem vorhandenen Aeronautik-Cache als `[{id, name, type?, icao_class?}]` (nach Name sortiert) — Auswahlquelle für den **AoR-Namens-Picker** im View-Editor. Kein neuer Fetch; leer, solange der Mandant keine OpenAIP-Daten hat; 404 wenn nicht verdrahtet. **admin** |
...
| `/api/admin/airports` | GET | **FR-UI-024:** ICAO-Flugplatzsuche für das View-Editor-Zentrum (`?q=<ICAO oder Name>&limit=`) → `[{icao, name, lat, lon}]`, gerankt exakter ICAO → Präfix → Namens-Substring; Query < 2 Zeichen → `[]`. Read-only Referenz aus dem **offline eingebetteten** Verzeichnis (`pkg/airport`, `go:embed airports.tsv`; Quelle: public-domain OurAirports, ~22 000 Plätze) — **kein** externer Dienst, **kein** Key; **admin** |
...
| `WAYFINDER_MAP_THEME` | `bkg-dark` | enum | `bkg-dark` (**Radar-Scope** auf amtlichen basemap.de-/basemap.world-Daten, server-seitig dunkel transformiert — Default) oder `bkg` (helle Variante). Die früheren Raster-Themes `dark` (CARTO) / `osm` (OSM) sind ausgebaut (ADR 0026 Nachtrag); die Namen wirken als deprecatete Aliase auf `bkg-dark`/`bkg` (Startup-Warnung) |
| `WAYFINDER_BKG_STYLE_URL` | basemap.world-„Farbe"-Style | URL | Upstream-Style für die `bkg`/`bkg-dark`-Themes. Default seit ADR 0026 Nachtrag basemap.world: `https://sgx.geodatenzentrum.de/gdz_basemapworld_vektor/styles/bm_web_wld_col.json` (innerhalb D amtliches basemap.de, außerhalb BKG-kuratierter Weltkontext aus OSM/NaturalEarth — kein leeres Umland mehr an der Staatsgrenze). Alternativen: Nur-Deutschland `…/gdz_basemapde_vektor/styles/bm_web_col.json`, Grau-Variante `bm_web_gry.json` oder self-hosted Mirror |
...
| `WAYFINDER_DWD_RADAR_STYLE` | (leer) | string | **#189:** optionaler WMS-`STYLES`-Parameter. Leer = Default-Style des Layers. Ein echo-only-Style (ohne Messbereichs-Grau/Stationsringe) lässt sich hier ohne Code-Änderung wählen (konkreter Style-Name gegen den DWD-GeoServer zu verifizieren) |
...
Platzhalter entfällt) (Fireflys Eingangs-Kontrakt ADR 0023, v1.5.0; `fireflySourcesEnv`
