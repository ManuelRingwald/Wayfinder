...
| `/api/airspace` | GET | Luftraumstrukturen (GeoJSON, best-effort). **ONB-6 (ADR 0011):** hinter der Tenant-Middleware; liefert den **Cache des Request-Mandanten** (eigener Schlüssel/AOI, Fallback auf den globalen Cache). **Feature-Gate:** ohne `airspaces`-Entitlement des Mandanten → **leere** Collection (Overlay erscheint nicht; server-seitig erzwungen). **Impersonation (ADR 0008 Nachtrag):** bei aktivem Grant zählt der **Ziel-Mandant** (Cache **und** Gate) |
