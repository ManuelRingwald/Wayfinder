...
| `/api/airspace` | GET | Luftraumstrukturen (GeoJSON, best-effort). **ONB-6 (ADR 0011):** hinter der Tenant-Middleware; liefert den **Cache des Request-Mandanten** (eigener Schlüssel/AOI, Fallback auf den globalen Cache). **Feature-Gate:** ohne `airspaces`-Entitlement des Mandanten → **leere** Collection (Overlay erscheint nicht; server-seitig erzwungen) |
| `/api/navaids` | GET | VOR/NDB-Beacons (GeoJSON, best-effort). **ONB-6:** mandanten-aufgelöst wie `/api/airspace`. **Feature-Gate:** ohne `vor_ndb` → leere Collection |
| `/api/waypoints` | GET | Wegpunkte (GeoJSON, best-effort). **ONB-6:** mandanten-aufgelöst wie `/api/airspace`. **Feature-Gate:** ohne `waypoints` → leere Collection |
